...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println("This sketch is for updating Arduino MKR and some other "
                 "ATSAMD21G18, 32-Bit ARM Cortex M0+ boards");
  Serial.println();
  ...
  Serial.print("SWD pad SWDRESET or RESET header pin connected to programmer "
               "board pin ");
  Serial.println(SWRST);
  Serial.println("Vcc (3.3V) pins should be connected between programmer and "
                 "target boards");
  Serial.print(
      "GND pins should be connected between programmer and target boards");
  Serial.println();
  Serial.println("See "
                 "https://gojimmypi.blogspot.com/2018/12/"
                 "swd-debugging-arduino-mkr-wifi-1010.html");
  Serial.println();
...
#define DEVICE_REV_MASK 0xf
  ...
  uint32_t mcu_id =
      dsu_did & DEVICE_ID_MASK; // 4 bits (bit 8-11) is just the revision
  ...
  while (selectedBoard == NULL) {
    ...
    Serial.println();
    for (const board_t *board = bootloaders; board->bootloader != NULL;
         board++) {
    ...
    Serial.println();
    Serial.print("Arduino IDE: Please enable No Line Ending with 115200 Baud "
  ...
  // Serial.print("Fuse high: 0x"); Serial.println((uint32_t)(fuses >> 32),
  // HEX); Serial.print("Fuse high: 0x");
  // Serial.println(dap._USER_ROW.fuseParts[1], HEX); Serial.print("Fuse low:
  // 0x"); Serial.println((uint32_t)(fuses & 0xFFFFFFFF), HEX);
  // Serial.print("Fuse low: 0x"); Serial.println(dap._USER_ROW.fuseParts[0],
  // HEX); Serial.print("Boot protect: 0x");
  // Serial.println(dap._USER_ROW.bit.BOOTPROT, HEX); Serial.print("Region lock:
  // 0x"); Serial.println(dap._USER_ROW.bit.LOCK, HEX);
  // Serial.print("BOD12_Config_Vreg: 0x");
  // Serial.println(dap._USER_ROW.bit._reserved_BOD12_Config_Vreg, HEX);
  // Serial.print("BOD12_Config: 0x");
  // Serial.println(dap._USER_ROW.bit._reserved_BOD12_Config, HEX); if
  // (dap._USER_ROW.fuses == 0ULL || dap._USER_ROW.fuses == ~0ULL) {
  //   // Entire User Row fuses shold never be 0 or all 1s
