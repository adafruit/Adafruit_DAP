/*
  ...
  Based on the webupdate example that comes with the ESP32 core and the flash
  from SD examples that come with the Adafruit_DAP library.

  This example let's you upload firmware to your DAP device directly from a
  browser. After compiling and uploading this example open your Serial Monitor
  and look for the following message: "Open http://esp-webdap.local or
  http://192.168.***.*** in your browser"

  If your system supports MDNS you can use the first address. Otherwise use the
  second. You will be greated by a file select field and a update button. Select
  your firmware and click select.

  The more advanced users can also upload through a terminal. Use: curl -F
  "update=@firmware.bin" esp-webdap.local/update
*/
...
const char *serverIndex =
    "<!DOCTYPE html><html><body><form method='POST' action='/update' "
    "enctype='multipart/form-data' onsubmit='fileCheck(event)'><input "
    "type='file' name='update' accept='.bin'><input type='submit' "
    "value='Update'></form><script>function "
    "fileCheck(e){document.getElementsByName('update')[0].files[0]||(alert('"
    "Select a file before clicking "
    "Update'),e.preventDefault())}</script></body></html>";
      ...
      []() {
        ...
        if (upload.status == UPLOAD_FILE_START) {
          ...
          for (unsigned int i = 0; i < upload.currentSize; i++) {
            ...
            // we will run this function once more in final for the last
            // bytes(if any)
            if (bufferd == BUFSIZE) {
              programDap();
            }
